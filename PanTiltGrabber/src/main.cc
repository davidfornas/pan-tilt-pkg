...
int main(int argc, char **argv) {
	...
	//ROS stuff
	ros::init(argc,argv,"PanTiltGrabber");
	ros::NodeHandle nh;
	image_transport::ImageTransport it(nh);
	image_transport::Publisher img_pub = it.advertise("/panTilt/camera", 1);
	ros::Publisher img_info_pub=nh.advertise<sensor_msgs::CameraInfo>("/panTilt/camera_info", 10);
...
//	m_avc->Play( m_port );
  ...
  img_info.distortion_model=std::string("plumb blob");
  img_info.D.resize(5);// = new float vector(5,0.0);
